...
public class CAWindow extends JFrame {
    ...
    private Vector<String> convertAndFilterForJtablePossibleSolution(
	    RankedSolution rs) {
    ...
    public void updatePossibleSolutions(EnsembleManager em,
	    List<RankedSolution> rs) {

	for (int i = 0; i < rs.size(); i++) {
	    Vector<String> row = convertAndFilterForJtablePossibleSolution(rs
		    .get(i));

	    ((DefaultTableModel) PossibleSolutionList.getModel()).addRow(row);
	    PossibleSolutionList.getSelectionModel().addListSelectionListener(
		    new RankedSolutionTableSelectionListener(
			    PossibleSolutionList, em, this));
	}
