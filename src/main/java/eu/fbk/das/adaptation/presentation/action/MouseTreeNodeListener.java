...
public class MouseTreeNodeListener extends MouseAdapter {
    ...
    private static final Logger logger = LogManager
	    .getLogger(MouseTreeNodeListener.class);
    ...
    public MouseTreeNodeListener(mxGraphComponent graphComponent,
	    List<EnsembleManager> ens, CAWindow window) {
	...
	if (!clicked && cell != null) {
		...
		if (((mxCell) cell).isVertex()) {
			...
			if (em.getDecisionPointSolutions() != null) {
			    for (Entry<String, List<RankedSolution>> entry : em
				    .getDecisionPointSolutions().entrySet()) {
				if (entry.getKey().equals(label)) {
				    // here we update the Possible Solutions
				    // window
				    window.updatePossibleSolutions(em,
					    entry.getValue());
				    found = true;
				    clicked = true;
				    break;
